...
class Database {
  ...
  async connect(type = `mongodb`) {
    if (process.env.NODE_ENV === "dev") { // Only enable for debugging in non-production env
      mongoose.set("debug", true);
      mongoose.set("debug", { color: true });
    }

    try {
      this.connection = await mongoose.connect(connectString, { maxPoolSize: 50 });
  ...
  async disconnect() {
    try {
      if (this.connection) { // Only disconnect if a connection exits 
